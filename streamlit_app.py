...
with col_logo2:
    if os.path.exists("marso_logo.png"):
        st.image("marso_logo.png", use_container_width=True)
    else:
    ...
    try:
        ...
        fish_chart = alt.Chart(fish_df).mark_area(
            ...
            x=alt.X('hour:Q', axis=alt.Axis(
                values=[0, 6, 12, 18, 24],
                labelExpr="datum.value == 0 ? '12 ص' : datum.value == 12 ? '12 م' : datum.value == 24 ? '12 ص' : datum.value == 6 ? '6 ص' : '6 م'",
                labelColor="#94A3B8", gridColor="rgba(255,255,255,0.05)", title=None, labelFont="Cairo", labelFontSize=12
            )),
