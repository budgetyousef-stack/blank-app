...
def fish_activity_score(wind: float, wave: float, temp: float, swell: float = 0.0) -> dict:
    ...
    elif wind <= 18: w_status, w_label, score = "#F59E0B", "مقبول", score - 20
    else: w_status, w_label, score = "#EF4444", "شديد", score - 50

    if wave <= 0.3: wv_status, wv_label = "#10B981", "هادئ"
    elif wave <= 0.7: wv_status, wv_label, score = "#F59E0B", "خفيف", score - 15
    else: wv_status, wv_label, score = "#EF4444", "عالي", score - 45

    if 22 <= temp <= 30: t_status, t_label = "#10B981", "مثالية"
    elif 18 <= temp <= 36: t_status, t_label, score = "#F59E0B", "مقبولة", score - 5
    else: t_status, t_label, score = "#EF4444", "قاسية", score - 20
...
# ─── تجميع الساعات النظيف لليوم الحالي فقط ──────────────────────────────────
def build_time_ranges(good_hours: list) -> list:
    ...
    for s, e in ranges:
        ...
        chips.append(fmt(s) if s == e else f"{fmt(s)} — {fmt(e + 1)}")
    return chips

# ─── نموذج المد والجزر التوافقي ───────────────────────────────────────────────
def _tide_amplitudes(lat: float, lon: float) -> dict:
    in_gulf = (23 <= lat <= 30) and (48 <= lon <= 57)
    if in_gulf: return dict(M2=0.90, S2=0.28, K1=0.12, O1=0.08, M4=0.06, base=1.10)
...
def compute_tide_profile(lat: float, lon: float, for_date: datetime) -> list:
    ...
    t0 = (midnight - datetime(2000, 1, 1)).total_seconds() / 3600.0
    ...
    for h in range(25):
        ...
        for name, period in periods.items():
            level += amp[name] * math.cos(2.0 * math.pi / period * t - phases[name])
...
def find_tide_events(heights: list) -> dict:
    ...
    for i in range(1, len(heights) - 1):
        if heights[i] >= heights[i-1] and heights[i] >= heights[i+1]: highs.append((i, heights[i]))
        elif heights[i] <= heights[i-1] and heights[i] <= heights[i+1]: lows.append((i, heights[i]))
...
def get_moon_phase(date: datetime) -> dict:
    ...
    age = (jdn - 2451549.26) % synodic
    illumination = int(round((1 - math.cos(2 * math.pi * age / synodic)) / 2 * 100))
    dtf, dtn = (14.765 - age) % synodic, (synodic - age) % synodic

    if age < 1.85: return {"age": age, "illumination": illumination, "emoji": "🌑", "phase_ar": "محاق — قمر جديد", "tidal": "مدّ ربيعي قوي 💪", "tidal_color": "#10B981", "tip": "أفضل وقت للصيد: المياه مظلمة والأسماك تتغذى بجرأة قرب السطح.", "tip_color": "#10B981", "days_to_full": dtf, "days_to_new": dtn}
    elif age < 7.38: return {"age": age, "illumination": illumination, "emoji": "🌒", "phase_ar": "هلال متصاعد", "tidal": "مدّ معتدل متصاعد", "tidal_color": "#F59E0B", "tip": "نشاط جيد: الأسماك في حالة بحث نشط عن الغذاء بعد المحاق.", "tip_color": "#F59E0B", "days_to_full": dtf, "days_to_new": dtn}
    elif age < 9.22: return {"age": age, "illumination": illumination, "emoji": "🌓", "phase_ar": "تربيع أول", "tidal": "مدّ محاقي ضعيف", "tidal_color": "#94A3B8", "tip": "نشاط متوسط: المدّ هادئ ومستقر، مناسب للصيد في الأعماق.", "tip_color": "#94A3B8", "days_to_full": dtf, "days_to_new": dtn}
    elif age < 14.76: return {"age": age, "illumination": illumination, "emoji": "🌔", "phase_ar": "أحدب متصاعد", "tidal": "مدّ متصاعد نحو الربيعي", "tidal_color": "#F59E0B", "tip": "نشاط متزايد: الأسماك تستعد لذروة نشاطها عند اكتمال البدر.", "tip_color": "#F59E0B", "days_to_full": dtf, "days_to_new": dtn}
    elif age < 16.61: return {"age": age, "illumination": illumination, "emoji": "🌕", "phase_ar": "بدر — قمر كامل", "tidal": "مدّ ربيعي قوي 💪", "tidal_color": "#10B981", "tip": "ذروة النشاط 🎣: أفضل ليلة للصيد! تيارات قوية والأسماك في أقصى نشاطها.", "tip_color": "#10B981", "days_to_full": dtf, "days_to_new": dtn}
    elif age < 22.15: return {"age": age, "illumination": illumination, "emoji": "🌖", "phase_ar": "أحدب متناقص", "tidal": "مدّ متناقص تدريجياً", "tidal_color": "#F59E0B", "tip": "نشاط جيد: القمر لا يزال مضيئاً والأسماك نشيطة.", "tip_color": "#F59E0B", "days_to_full": dtf, "days_to_new": dtn}
    elif age < 23.99: return {"age": age, "illumination": illumination, "emoji": "🌗", "phase_ar": "تربيع أخير", "tidal": "مدّ محاقي ضعيف", "tidal_color": "#94A3B8", "tip": "نشاط معتدل: المدّ محاقي هادئ، أفضل للصيد الصباحي الهادئ.", "tip_color": "#94A3B8", "days_to_full": dtf, "days_to_new": dtn}
    else: return {"age": age, "illumination": illumination, "emoji": "🌘", "phase_ar": "هلال متناقص", "tidal": "مدّ ضعيف يتراجع", "tidal_color": "#64748B", "tip": "نشاط منخفض: القمر في تراجعه الأخير نحو المحاق القادم.", "tip_color": "#64748B", "days_to_full": dtf, "days_to_new": dtn}

# ─── جلب البيانات ───────────────────────────────────────────────────────────
@st.cache_data(ttl=1800)
def fetch_weather(lat: float, lon: float):
    w_url, m_url = "https://api.open-meteo.com/v1/forecast", "https://marine-api.open-meteo.com/v1/marine"
    w_params = {"latitude": lat, "longitude": lon, "hourly": "temperature_2m,apparent_temperature,windspeed_10m,windgusts_10m,winddirection_10m,precipitation_probability,visibility,cloudcover", "daily": "windspeed_10m_max,windgusts_10m_max,temperature_2m_max,temperature_2m_min,precipitation_probability_max", "wind_speed_unit": "kmh", "forecast_days": 7, "timezone": "Asia/Riyadh", "models": "best_match"}
    m_params = {"latitude": lat, "longitude": lon, "hourly": "wave_height,wave_direction,wave_period,wind_wave_height,wind_wave_period,swell_wave_height,swell_wave_period,swell_wave_direction,sea_surface_temperature", "daily": "wave_height_max,wave_period_max,swell_wave_height_max,wind_wave_height_max", "forecast_days": 7, "timezone": "Asia/Riyadh", "length_unit": "metric", "models": "best_match"}
    try: return requests.get(w_url, params=w_params, timeout=12).json(), requests.get(m_url, params=m_params, timeout=12).json()
...
if st.session_state["coords"]: display_text = f"📍 الموقع المحدد: {get_location_name(flat, flon)} ({flat:.3f}°، {flon:.3f}°)"
else: display_text = "📍 انقر على الخريطة لتحديد موقعك البحري"
...
m = folium.Map(location=[flat, flon], zoom_start=9, tiles="CartoDB dark_matter", prefer_canvas=True)
m.get_root().html.add_child(folium.Element("<style>html,body,.leaflet-container,.leaflet-pane,.leaflet-tile { direction: ltr !important; text-align: left !important; }</style>"))

if st.session_state["coords"]:
    folium.Marker(location=[flat, flon], icon=folium.Icon(color="blue", icon="anchor", prefix="fa")).add_to(m)
    folium.Circle(location=[flat, flon], radius=5000, color="#38BDF8", fill=True, fill_color="#38BDF8", fill_opacity=0.10, weight=2).add_to(m)
...
if map_data and map_data.get("last_clicked"):
    ...
    if st.session_state["coords"] != nl:
        st.session_state["pending_coords"] = nl
        st.rerun()

# ─── التحليلات ────────────────────────────────────────────────────────────────
if st.session_state["coords"]:
    w_res, m_res = fetch_weather(flat, flon)
    if w_res is None or m_res is None or "hourly" not in w_res:
        ...
        try:
            ...
            def safe_get(lst, idx, default=0.0):
                try: return lst[idx] if lst[idx] is not None else default
            ...
            # تعديل صارم: البحث في ساعات اليوم الحالي المتبقية فقط حتى منتصف الليل (ساعة 24)
            look_ahead = 24
            good_hours = [i for i in range(curr_h, look_ahead) if safe_get(hw.get("windspeed_10m", []), i) <= 15 and safe_get(hm.get("wave_height", []), i) <= 0.7 and safe_get(hw.get("precipitation_probability", []), i, 0) < 40]
            ...
            <div class="card">
              ...
              <div class="metric-row">
                ...
                <div class="metric-box"><div class="metric-value">{wind_gust:.0f} كم/س</div><div class="metric-label">أقصى هبّة</div></div>
              </div>
              <div class="metric-row">
                <div class="metric-box"><div class="metric-value">{wave_now:.1f} م</div><div class="metric-label">ارتفاع الموج</div></div>
                <div class="metric-box"><div class="metric-value">{wave_period:.0f} ث</div><div class="metric-label">دورة الموج</div></div>
                <div class="metric-box"><div class="metric-value">{swell_now:.1f} م</div><div class="metric-label">التورم البحري</div></div>
                <div class="metric-box"><div class="metric-value">{sst:.1f}°م</div><div class="metric-label">حرارة السطح</div></div>
              </div>
            ...
            if time_ranges:
                ...
                st.markdown(f'<div style="margin-top:-8px; margin-bottom:16px; direction:rtl; text-align:right;"><span style="color:#94A3B8; font-size:0.85rem; margin-left:8px;">🕐 أفضل فترات الصيد المتبقية اليوم:</span>{chips_html}</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div style="margin-top:-8px; margin-bottom:16px; color:#EF4444; font-size:0.85rem; text-align:right;">⚠️ لا توجد فترات صيد مناسبة متبقية اليوم.</div>', unsafe_allow_html=True)
            ...
            <div class="card">
              ...
              <div style="text-align:center;padding:14px;background:rgba(255,255,255,0.04);border-radius:12px;border:2px solid {act['color']};color:{act['color']};font-size:1.5rem;font-weight:900;">{act['label']}</div>
              <div class="activity-labels"><span>منخفض 0%</span><span>متوسط 50%</span><span>100% عالي</span></div>
              ...
              <div style="text-align:center;color:#94A3B8;font-size:0.82rem;margin-bottom:10px;">مؤشر النشاط: <strong style="color:{act['color']}">{act['score']}%</strong></div>
              <div style="background:rgba(255,255,255,0.03);border-radius:10px;padding:12px;color:#CBD5E1;text-align:right;font-size:0.9rem;line-height:1.7;">{act['advice']}</div>
              <div class="factor-row">
                <div class="factor-box"><div class="factor-value" style="color:{act['wind_status']}">{act['wind_label']}</div><div class="factor-label">الرياح</div></div>
                <div class="factor-box"><div class="factor-value" style="color:{act['wave_status']}">{act['wave_label']}</div><div class="factor-label">الموج</div></div>
                <div class="factor-box"><div class="factor-value" style="color:{act['temp_status']}">{act['temp_label']}</div><div class="factor-label">الحرارة</div></div>
              </div>
            </div>
            """, unsafe_allow_html=True)

            # ─── المنحنى البياني للمد والجزر ───
            try:
                tide_heights = compute_tide_profile(flat, flon, datetime.now())
                tide_chart_data = pd.DataFrame({"المد (م)": [float(v) for v in tide_heights]}, index=list(range(25)))
                st.markdown('<div class="map-section-label" style="margin-top:4px;">📉 منحنى المد والجزر — 24 ساعة</div>', unsafe_allow_html=True)
                st.line_chart(tide_chart_data, height=175, use_container_width=True, color=["#38BDF8"])
                st.markdown(f'<div style="text-align:right;font-size:0.72rem;color:#475569;margin-top:2px;">المحور الأفقي: ساعات اليوم (0–24) &nbsp;|&nbsp; <span style="color:#38BDF8;">■</span> منسوب المد &nbsp;| الساعة الحالية: {fmt_tide_hour(curr_tide_h)}</div>', unsafe_allow_html=True)
            except: pass
                ...
                <div class="card">
                  ...
                  <div class="moon-row">
                    ...
                    <div class="moon-info">
                      ...
                      <div style="margin-top:8px;"><span style="background:{moon["tidal_color"]}22;color:{moon["tidal_color"]};border:1px solid {moon["tidal_color"]}55;border-radius:16px;padding:3px 12px;font-size:0.78rem;font-weight:800;">{moon["tidal"]}</span></div>
                    </div>
                  </div>
                  <div class="moon-events-row">
                    <div class="moon-event-box"><div class="moon-event-label">🌕 حتى البدر القادم</div><div class="moon-event-val">{round(moon["days_to_full"], 1)} يوم</div></div>
                    <div class="moon-event-box"><div class="moon-event-label">🌑 حتى المحاق القادم</div><div class="moon-event-val">{round(moon["days_to_new"], 1)} يوم</div></div>
                    <div class="moon-event-box"><div class="moon-event-label">📅 الدورة القمرية</div><div class="moon-event-val">29.5 يوم</div></div>
                  </div>
                  <div class="moon-tip" style="border-right-color:{moon["tip_color"]};">{moon["tip"]}</div>
                </div>
                """, unsafe_allow_html=True)
            except: pass
            ...
            try:
                dw, dt, dwv, dsw = w_res["daily"].get("windspeed_10m_max", [0]*7), w_res["daily"].get("temperature_2m_max", [0]*7), m_res["daily"].get("wave_height_max", [0]*7), m_res["daily"].get("swell_wave_height_max", [0]*7)
                lbls = ["اليوم" if i == 0 else DAYS_AR[(datetime.now() + timedelta(days=i)).weekday() % 7] for i in range(min(7, len(dw)))]
                chart_data = pd.DataFrame({"اليوم": lbls, "الرياح (كم/س)": [round(v) for v in dw[:len(lbls)]], "الأمواج (م)": [round(v, 2) for v in dwv[:len(lbls)]], "الحرارة (°م)": [round(v) for v in dt[:len(lbls)]], "التورم البحري (م)": [round(v, 2) for v in dsw[:len(lbls)]]}).set_index("اليوم")
                st.markdown('<div class="map-section-label" style="margin-top:4px;">📈 توقعات الأسبوع القادم</div>', unsafe_allow_html=True)
                st.line_chart(chart_data, height=220, use_container_width=True, color=["#38BDF8", "#10B981", "#F59E0B", "#A78BFA"])
            except: pass

        except Exception as e: st.error("⚠️ يرجى اختيار نقطة داخل البحر لتشغيل التحليلات.")

st.markdown('<div class="footer">Marine Tracker PRO • بيانات Open-Meteo • الخرائط OpenStreetMap</div>', unsafe_allow_html=True)
